...
function jump(hash: string) : void { location.replace("#" + hash) }
...
class Image {
    ...
    public async compress(quality: number, onProgress: (progress: number) => void) : Promise<void> {
        try {
            ...
            this.compressedData = await imageCompression.getDataUrlFromFile(this.compressedFile)
    ...
    public createView(index: number, total: number) : string {
        ...
        <div id="item-${index}" class="carousel-item">
            <h2 style="margin: 0;">Image ${index+1} of ${total}<span> was compressed from ${this.originalSize} to ${this.newSize}</span></h2>
            ...
            <a class="arrow-container arrow-container-prev" href="#item-${index == 0 ? total - 1 : index - 1}"><div class="arrow arrow-prev"></div></a>
            <a class="arrow-container arrow-container-next" href="#item-${index == total - 1 ? 0 : index + 1}"><div class="arrow arrow-next"></div></a>
            </div>
        </div>
        `
    }
...
function updateState(doingAnything: boolean) {
    ...
    imagesInput.disabled = levelInput.disabled = saveButton.disabled = doingAnything
}

function updateViewSaving(saving: boolean) : HTMLProgressElement {
    updateState(saving)
    const savingProgress = document.getElementById('savingProgress') as HTMLProgressElement
...
function updateViewCompressing(compressing: boolean) : HTMLProgressElement {
    ...
    const compressingProgress = document.getElementById('compressingProgress') as HTMLProgressElement
    compressingProgress.style.display = compressing ? 'block' : 'none'
    if (!compressing) {
        const afterCompressionStyle = document.getElementById('afterCompression') as HTMLDivElement
...
function sleep(duration: number): Promise<unknown> {
...
async function addImages(event: Event) {
    ...
    const images = (currentImages.length > 0 
        ? files.filter(v => !currentImages.some(k => k.originalFile.name == v.name)) 
        : files).map(v => new Image(v))
    if (images.length == 0) {
        alert('Simultaneous compression of images with the same name is not possible.')
        return
    }
...
async function compressImages(event: Event) {
    ...
    const images = currentImages.filter(v => v.currentCompressedQuality != quality)
    if (images.length == 0) {
    ...
    const resultView = currentImages.map((v,i) => v.createView(i, currentImages.length)).join('')
